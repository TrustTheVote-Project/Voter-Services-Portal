class RegistrationsController < ApplicationController
  ...
  def finalize_create(active_form, reg = @registration, ses = session)
    ...
    paperless = AppConfig['dmv_ovr'] && submitted && reg.paperless_submission_allowed? && reg.dmv_id.present?
  ...
  def finalize_update(active_form, reg = @registration, ses = session)
    ...
    if !AppConfig['ssn_required'] || reg.ssn.present?
      begin
        ...
        Rails.logger.error "INTERNAL ERROR: SUBMIT_EML310 - Failed to update: vid=#{reg.voter_id} code=#{e.code} message=#{e.message}" if AppConfig['api_debug_logging']
      end
    end
    ...
    return AppConfig['dmv_ovr'] && submitted && reg.paperless_submission_allowed? && reg.dmv_id.present?
  end
